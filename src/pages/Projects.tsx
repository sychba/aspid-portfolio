        ...
        <div className="flex flex-col gap-8">
          {projects ? (
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className={cn(
                        ...
                        project.status?.toLowerCase().includes("unfinished") || project.status?.toLowerCase().includes("abandonded")
                          ? "bg-amber-500/10 text-amber-500 border-amber-500/20"
                          : project.status?.toLowerCase().includes("completed") || project.status?.toLowerCase().includes("finished")
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {project.tags.map((tag) => (
                        <span key={tag} className="px-2.5 py-1 text-xs rounded-md bg-white/5 text-white/70 border border-white/5">
                          {tag}
                        </span>
                      ))}
                    </div>
            ...
            Array.from({ length: 3 }).map((_, i) => (
              <GlassCard key={i} className="h-64 animate-pulse flex flex-col md:flex-row">
